...
export function envParseInteger(key: EnvInteger, defaultValue?: number) {
  ...
  if (isNullishOrEmpty(value)) {
    if (defaultValue === undefined)
      throw new TypeError(
        `EnvError: "${key}" must be an integer, but is empty or undefined.`
      );
    return defaultValue;
  }
  ...
  throw new Error(
    `EnvError: "${key}" must be an integer, but received "${value}" (${typeof value}).`
  );
}
...
export function envParseBoolean(key: EnvBoolean, defaultValue?: boolean) {
  ...
  if (isNullishOrEmpty(value)) {
    if (defaultValue === undefined)
      throw new TypeError(
        `EnvError: "${key}" must be a boolean, but is empty or undefined.`
      );
    return defaultValue;
  }
  ...
  throw new Error(
    `EnvError: "${key}" must be a boolean, but received "${value}" (${typeof value}).`
  );
}

export function envParseString<K extends EnvString>(
  key: K,
  defaultValue?: KRLSEnv[K]
) {
  const value = process.env[key];
  ...
  if (isNullishOrEmpty(value)) {
    if (defaultValue === undefined)
      throw new TypeError(
        `EnvError: "${key}" must be a string, but is empty or undefined.`
      );
    return defaultValue;
  }
...
export function envParseArray(key: EnvString, defaultValue?: string[]) {
  ...
  if (isNullishOrEmpty(value)) {
    if (defaultValue === undefined)
      throw new Error(
        `EnvError: "${key}" must be an array, but is empty or undefined.`
      );
    return defaultValue;
  }
